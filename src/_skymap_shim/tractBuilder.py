...
class LegacyTractBuilder(BaseTractBuilder):
    ...
    def __init__(self, config):
        ...
        self._patchInnerDimensions = geom.Extent2I(*(val for val in config.patchInnerDimensions))
    ...
    def getPatchInfo(self, index, tractWcs):
        ...
        if isinstance(index, Index2D):
            ...
            if isinstance(index, numbers.Number):
                ...
                _index = Index2D(*index)
        if (not 0 <= _index.x < self._numPatches.x) or (not 0 <= _index.y < self._numPatches.y):
            raise IndexError(
...
class CellTractBuilder(BaseTractBuilder):
    ...
    def __init__(self, config):
        ...
        self._cellInnerDimensions = geom.Extent2I(*(val for val in config.cellInnerDimensions))
        ...
        self._patchInnerDimensions = geom.Extent2I(
            *(val * self._numCellsPerPatchInner for val in config.cellInnerDimensions)
        )
