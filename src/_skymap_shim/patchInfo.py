...
from .utils import makeSkyPolygonFromBBox, Index2D
...
class PatchInfo:
    ...
    def __init__(
        ...
        if numCellsPerPatchInner == 0:
            ...
            self._numCells = Index2D(
                x=numCellsPerPatchInner + 2 * numCellsInPatchBorder,
    ...
    def getCellInfo(self, index):
        ...
        if (not 0 <= _index.x < self._numCells.x) or (not 0 <= _index.y < self._numCells.y):
            raise IndexError(
        ...
        innerMin = Point2I(
            *[
                (_index[i] - self._numCellsInPatchBorder) * self._cellInnerDimensions[i]
